...
# Plot average signal per each category defined in DE_results_df['agg_key'] over the matrix window 
def plot_avg_signal(DE_results_df, stackup_matrix, title, ax=None, DE_value_col='log2Fold_Change',
                    cutoff='padj', cutoff_val=0.05, 
                    agg_key='DE_status', agg_categories=['up', 'down', 'nonsig'], 
                    color_categories=['r', 'b', 'k'], windowSize=1000, nbins=40):
...
def plot_binned_signal_heatmap(DE_results_df, stackup_matrix, title, ax=None,
                               DE_value_col='log2FoldChange', sort_by_DE=True, 
                               agg_key='DE_status', agg_categories=['up', 'down'],
                               include_category_map=True, color_categories=['r', 'b'], 
